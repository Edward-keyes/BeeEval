...
import com.xailab.vehicle.framework.common.excel.ExcelDataListener;
...
public class ExcelUtils {
     ...
     */
    public static <T> List<T> syncReadModel(MultipartFile file, Class<T> clazz , Integer sheetNo, Integer headRowNum){
        try {
            return EasyExcelFactory.read(file.getInputStream()).sheet(sheetNo).headRowNumber(headRowNum).head(clazz).doReadSync();
        } catch (IOException e) {
            log.error("syncReadModel error", e);
            throw new ServerException("解析excel异常");
        }
    }
     ...
     * @param data      数据
     */
    public static <T> void excelExport(Class<T> head, String excelName, String sheetName, List<T> data,HttpServletResponse response) {
        try {
            getExportResponse(excelName,response);
            EasyExcel.write(response.getOutputStream(), head)
                    .autoCloseStream(Boolean.FALSE)
                    .sheet(StringUtils.isBlank(sheetName) ? "sheet1" : sheetName)
                   .doWrite(data);
    ...
    private static void getExportResponse(String excelName,HttpServletResponse response) {
